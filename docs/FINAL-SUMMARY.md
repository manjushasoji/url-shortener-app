...
Static quality gates: every push and PR runs `./mvnw verify` in GitHub Actions — Checkstyle (hygiene rules, `config/checkstyle.xml`), the full test suite against a real MySQL 8 container, then SpotBugs (`Max` effort, `Medium` threshold, suppressions with stated reasons in `config/spotbugs-exclude.xml`) — and any violation fails the build. Every code change in this repo was run locally (`./mvnw test`, against a live MySQL 8) by the engineer before it was committed, and then again by CI on the pull request — no change was merged on the strength of reading the code alone. Several entries in [AI-TRACEABILITY.md](AI-TRACEABILITY.md) (items 5–6) are exactly the cases where that local run caught a defect the AI's own reasoning had missed. `main` is green. Full per-area breakdown: README **Testing**.
