...
- **`ApiErrorController`** (implements `ErrorController`, mapped to `/error`) — replaces Spring Boot's default error handling for anything that never reaches a controller method at all (most notably an unmapped path), so it returns the same `ApiError` JSON shape as every other error instead of the whitelabel page or Spring's own differently-shaped error JSON. Covers the whole app, not just `/api/v1/**`.
...
- `ApiErrorController`'s `message` field is Spring Boot's own generic wording, not a purpose-written message (see Known Limitations in the README)
