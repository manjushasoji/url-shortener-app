...
Any path that doesn't match a route at all — a typo, a made-up endpoint, anywhere in the app, not just under `/api/v1` — returns the same `ApiError` JSON shape instead of an inconsistent response format. Two mechanisms are involved, and only one of them fires for the common case: as of Spring Framework 6.1 (bundled with this project's Spring Boot 3.4.1), an unmatched `GET`/`HEAD` request throws `NoResourceFoundException` *through the normal controller-advice flow*, so `GlobalExceptionHandler.handleNoResourceFound` catches it directly — this is the one that actually runs. `ApiErrorController` (mapped to `/error`) is a separate fallback for errors that occur outside controller-advice handling entirely (e.g. before `DispatcherServlet` dispatch); it's not reached for a typical unmapped-path request, but is kept as defense-in-depth for whatever else might land on `/error`.
